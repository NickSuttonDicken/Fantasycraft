...
public class TritonTrident extends TridentItem {
    private final Multimap<EntityAttribute, EntityAttributeModifier> attributeModifiers;

    public TritonTrident(Settings settings){
        super(settings);
        ...
        builder.put(EntityAttributes.GENERIC_ATTACK_SPEED, new EntityAttributeModifier(ATTACK_SPEED_MODIFIER_ID, "Tool modifier", -2.5D, EntityAttributeModifier.Operation.ADDITION));
            ...
            if (i >= 10) {
                ...
                if (riptideLevel <= 0 || playerEntity.isTouchingWaterOrRain() || playerEntity.isInLava()) {
                    if (!world.isClient) {
                        ...
                        if (riptideLevel == 0) {
                            TritonTridentEntity tridentEntity = new TritonTridentEntity(world, playerEntity, stack);
